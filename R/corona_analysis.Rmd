...
data <- 
  local({
    ...
    df_proc <- 
      ...
      arrange(date) %>%
      mutate(
        new_cases = max(confirmed - lag(confirmed),0),
    ...
    df_clean <- 
      ...
      select(country, country_state, n_day, date, restrict_date, everything())
...
# Total confirmed cases by day since case 1.
...
local({
    ...
    mutate(n_day_restrict = min(n_day[date == restrict_date], na.rm = T),
           country_state_restrict_date = paste0(country_state,": ", restrict_date)) %>% 
    ungroup()
  
  plot <- 
    ggplot(data = data_for_plot,
           aes(x = n_day,y = confirmed, date = date, color = country_state),
           show.legend = FALSE) +
...
local({
  ...
  plot <- 
    ...
    geom_point(alpha = 0.6, color = "black", show.legend = FALSE) +
    geom_line(alpha = 0.6,show.legend = FALSE) +
    ggtitle("Log(confirmed) by date country") +
    geom_vline(aes(xintercept = restrict_date),
               show.legend = FALSE)  +
