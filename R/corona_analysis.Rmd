  ...
  local({
    ...
    interpolation <- 
     ...
     extrapolation <- 
      ...
      # 
      # 
      # mutate(confirmed_model = Reduce(init = df_latest_day_IL$confirmed,
      #        x = lead(r_model), 
     ...
     model_data <- 
       ...
       group_by(country_state) %>% 
       mutate(confirmed_latest = max(confirmed[which.max(date)]),
              confirmed_model = 
                Reduce(init = max(confirmed_latest),
                       x = lead(r_model),
                       f = prod, accumulate = T) %>% head(-1)) %>% 
       select(
         country, country_state, date, date0, n_day, confirmed, new_cases, 
         r, fit_type, r_model, prc_change_in_total_cases, 
         date_peak,confirmed_latest,confirmed_model
       )
  })
...
local({
  ...
  plot <- 
    ...
    ggtitle("Total confirmed cases by day") +
...
# for each <geo,time> calc 7 days fwd r_model 
...
ts_forecast <- 
  ...
  ungroup() %>% 
  arrange(country_state, date) %>% 
  select(country_state, date, r_model, date_il, confirmed_forecast_isr)

ts_forecast_with_il <- 
  ts_forecast %>% 
  select(country_state, date_il, confirmed_forecast_isr) %>% 
  bind_rows(model_data %>% 
              filter(country_state == "Israel") %>% 
              select(country_state, date, confirmed)
  ) 
...
local({
  ...
  plot <- 
    ggplot(model_data,
           aes(x = date,y = prc_change_in_total_cases, color = country_state),
           show.legend = FALSE) +
    geom_point(alpha = 0.4, color = "black") +
    geom_line(
      data = model_data %>% filter(fit_type=="interpolation"),
      aes(x=date,y=prc_change_in_total_cases, color = country_state), 
      show.legend = FALSE,
      alpha = 0.8, size = 1.0) +
    geom_line(
      data = model_data %>% filter(country_state =="Israel", fit_type == "extrapolation"),
      aes(x = date,y = prc_change_in_total_cases),
      show.legend = FALSE,
      linetype = "dashed"
      ) +
    ggtitle("% change in total cases") +
