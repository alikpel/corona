    ...
    interpolation <- 
     ...
     model_data <- 
       ...
       mutate(confirmed_latest = max(confirmed[which.max(date)]),
        ...
        mutate(date_latest = date[which.max(date)],
               n_day_latest = n_day[which.max(date)],
              date = date_latest + days(n_day-n_day_latest))  %>% # add date to extrapolated values
       ungroup() %>% 
       select(
         country,
         country_state, 
...
local({
  ...
  plot <- 
    ggplot(model_data %>% filter(fit_type=="interpolation"),
    ...
    geom_line( #forecast of raw
      data = model_data %>% filter(fit_type == "extrapolation"),
      aes(x = date,y = prc_change_in_total_cases_model),
      show.legend = FALSE,
      linetype = "dashed"
      ) +
    geom_vline(data = ts_forecast_with_il %>% filter(country_state != "Israel"), aes(xintercept = dateStart))+
    geom_vline(data = ts_forecast_with_il %>% filter(country_state != "Israel"), aes(xintercept = dateEnd))+
    ...
    #   size = 1
    #   ) +
    ggtitle("Date range selected for IL forecasting") +
    ylim(0, 70) +
    ylab("% change in total cases") +
    facet_wrap(~country_state, scales = "free") 
...
# figure 2: full IL + prediction by location
