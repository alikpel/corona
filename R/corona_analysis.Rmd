  ...
  local({
    ...
    df_proc <- 
      ...
      mutate(
        ...
        date0 = min(date[date > date_peak + 7 & date > date_case100]),
    ...
    df_clean <- 
      df_proc %>% 
      filter_verbose(
        (n_country_state_confirmed >= 1000 | country == "Israel")
        ) %>% 
      group_by(country_state) %>% 
  ...
  local({
    ...
    interpolation <- 
     ...
     model_data <- 
       ...
       mutate(prc_change_in_total_cases = r_model - 1)
  })
...
# model - daily % change (r = 1 - number total cases stays the same; r > 1 proportional to growth rate, e.g. if in day i r=1.2 it means that the number of total patients grew by 20% relative to the day i - 1 ). the dashed line show extrapulation 7 days forward.
# Day 0 - the first day since the peak in r when total number of patients > 100 and 
model_data %>% 
  filter(n_day>=0, fit_type=="interpolation") %>% 
  ggplot(aes(x=n_day,y=prc_change_in_total_cases, color = country_state)) +
  geom_point(alpha = 0.2) +
  geom_line() +
  geom_line(aes(y=prc_change_in_total_cases), 
            data = model_data %>% filter(n_day>=0, fit_type=="extrapolation"),             
            linetype = "dashed") +
  ggtitle("% change in total cases by day",subtitle = "Day 0 - is the first day with after the data is stabilized") +
  ylim(0,.6) +
  ylab("% change in total cases") +
...
#   tsbl %>% 
#   autoplot(r) +
#   geom_line(data = augment(ETS_fitted), linetype = "dashed")
# ) %>% ggplotly()
